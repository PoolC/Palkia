...
public class RoomService {
    ...
    public void updateReservation(RoomReservation reservation, Long id){
        Room room = roomRepository.findRoomReservationById(id).orElseThrow(() -> new RuntimeException("해당하는 id의 예약이 없습니다."));
        //check host
        if(reservation.getHost().equals(room.getHost())){
            ...
            throw new RuntimeException("내가 한 예약이 아닙니다.");
        }
    }

    public void cancelReservation(Member member, Long reservationId){
        Room room = roomRepository.findRoomReservationById(reservationId).orElseThrow(() -> new RuntimeException("해당하는 예약이 존재하지 않습니다."));
    ...
    private void RequestValidCheck(RoomReservation roomReservation){
        ...
        if(roomReservation.getStart().getMinute()%30!=0 || roomReservation.getEnd().getMinute()%30!=0){
            throw new RuntimeException("30분 단위로만 예약할 수 있습니다.");
        }
    }
